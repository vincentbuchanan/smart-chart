...
server.post("/chart/", function(req, res, next) {
  const error = validateChart(req.body);
  if (error) {
...
function validatePatient(patient) {
  if (!patient.ssn) return "SSN is required.";
  if (!patient.patientRef) return "PatientRef is required.";
  if (!patient.id) return "ID is required.";
  return "";
}

function validateChart(chart) {
  if (!chart.id) return "ID is required.";
  if (!chart.patientId) return "Patient ID is required.";
  return "";
}
